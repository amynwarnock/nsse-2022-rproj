...
date: "Last updated November 29, 2022"   
output:
  html_document:
    toc: true
    toc_depth: 5
...
# load Fall 2022 ENR census file (created by JP at IR, rec'd 11/21/2022)

enr_orig_full <- import(
  here("data", "enr_detail_fall_2022.xlsx"),
       setclass = "tbl_df")
```
...
# counts and distributions of tmworkhrs and tmworkhrs_binned
nsse_chr_wide %>% 
  group_by(tmworkhrs_binned) %>% 
  summarize(n = n()) %>% 
  ggplot(aes(x = tmworkhrs_binned, y = n))  + 
  geom_col()

nsse_chr_wide %>% 
  group_by(tmworkhrs_binned) %>% 
  summarize(n = n()) %>% 
  ggplot(aes(x = reorder(tmworkhrs_binned, n), y = n))  + 
  geom_col()

nsse_chr_wide %>% # Majority of FY students do not work. Majority of SR students work 11-20 hrs/wk.
  ggplot(aes(x = tmworkhrs)) +
  geom_bar() +
  facet_wrap(~IRclass)

nsse_chr_wide %>% 
  ggplot(aes(x = tmworkhrs_binned)) +
  geom_bar() +
  facet_wrap(~IRclass)

# nsse_fct_wide %>% 
#   filter(!is.na(tmworkhrs_binned)) %>% 
#   ggplot(aes(x = tmworkhrs_binned)) +
#   geom_bar(aes(y = (..count..)/sum(..count..))) +
#   facet_wrap(~IRclass)

nsse_chr_wide %>% 
  group_by(tmworkhrs_binned, IRclass) %>%
  filter(!is.na(tmworkhrs_binned)) %>% 
  summarize(n = n()) %>% 
  pivot_wider(names_from = tmworkhrs_binned,
              values_from = n) 


table1 <- xtabs(~ IRclass + tmworkhrs_binned, data = nsse_chr_wide)

prop.table(table1, 1)

prop.table(nsse_chr_wide$IRclass, nsse_chr_wide$tmworkhrs_binned)
```

#### Check that tmds, tmworkhours, tmcocurr variables were recoded correctly

```{r check-recoding}

# check the recoded tmcocurr, tmworkhrs, and race/eth variables
...
ftable(nsse_chr_wide$tmworkhrs, nsse_chr_wide$tmworkhrs_binned, exclude = "")

nsse_chr_wide %>% 
  filter(is.na(tmworkhrs)) %>% 
  summarize(n = n())
...
#### Pivot longer for dumbbell graphs and summary tables
...
# Cognos demographics dataset: select vars that will be used in these analyses, recode residency status, and create variable for first gen status (i.e., parent education level)
...
#### Wrangle ENR file - retention and graduation vars

```{r}

enr_ret_grad <- enr_orig_full %>% 
  select(PIDM,
         ID,
         ret_fall_term_yr2,
         ret_fall_term_yr3,
  ...
  mutate(
    ttd_4 = ifelse(rowSums(across(starts_with("grad") &
                                    ((ends_with("yr1") |
                                     ends_with("yr2") |
                                     ends_with("yr3") |
                                     ends_with("yr4"))), 
                   ...
                   1,
                   0),
    ttd_5 = ifelse(rowSums(across(starts_with("grad") &
                                    (ends_with("yr5")), 
                                  ~.x == "1")) >= 1,
                   1,
                   0),
    ttd_6 = ifelse(rowSums(across(starts_with("grad") &
                                    (ends_with("yr6")), 
                                  ~.x == "1")) >= 1,
                   1,
                   0),
    ttd_7 = ifelse(rowSums(across(starts_with("grad") &
                                    (ends_with("yr7") |
                                     ends_with("yr8") |
                                     ends_with("yr9") |
                                     ends_with("yr10")), 
                   ...
                   1,
                   0),
    ttd_count = rowSums(across(ttd_4:ttd_7)),
    ttd = case_when(
      ttd_4 == 1 ~ '4',
      ttd_5 == 1 ~ '5',
      ttd_6 == 1 ~ '6',
...
# checking to see if the issue of students w/ consecutive graduation indicators occurs in winter 2022 ENR file
enr_ret_grad_winter2022 <- import(
  here("data", "enr_detail_202102_winter_2022.xlsx"),
       setclass = "tbl_df") %>% 
  select(PIDM,
         ID,
         ret_fall_term_yr2,
...
enr_winter22_multgrad <- enr_ret_grad_winter2022 %>% 
  filter(ttd_4 == "1" &
           ttd_5 == "1" &
           ttd_6 == "1" & 
           ttd_7 == "1")

enr_fall22_multgrad <- enr_ret_grad %>% 
  filter(ttd_4 == "1" &
           ttd_5 == "1" &
           ttd_6 == "1" & 
           ttd_7 == "1")

enr_fall22_multgrad_winterID <- enr_winter22_multgrad %>% 
  select(PIDM, ID) %>% 
  mutate(winter22 = "x") %>% 
  right_join(enr_fall22_multgrad,
             by = "PIDM") %>%
  select(-starts_with("grad"))
    
#nsse_enr_testing <- left_join(nsse_fct_wide, enr_ret_grad,
                              

xtabs(~ IRclass + ret_fall_term_yr2, data = nsse_enr_testing)

fy <- nsse_enr_testing %>% 
  filter(IRclass == "First Year")
  
  
fy_tmwork <- xtabs(~ tmworkhrs_binned + ret_fall_term_yr2, data = fy)

prop.table(fy_tmwork, 1)


fy_tmcocurr <- xtabs(~ tmcocurr_binned + ret_fall_term_yr2, data = fy)

prop.table(fy_tmcocurr, 1)

nsse_fct_wide$tmworkhrs_binned
```
...
dumbbellprep_comp <- function(data) {
...
# dumbbell v3 (mean score, all student subgroups on on y-axis, color by FY/SR)

dumbbellplot_comp <- function(data, comp_title, comp_subtitle, comp_caption) {
...
note_gender <- ""
  ...
  #"Note. Binary gender is reported due to how this information is collected during admissions and to protect student privacy and confidentiality."

note_tmds <- ""
  
  #"Note. TMDS = Traditionally Marginalized Domestic Students. TMDS/Non-TMDS calculated based on race/ethnicity data collected during admissions. TMDS includes students who reported their race/ethnicity as American Indian or Alaska Native, Asian, Black or African American, Hispanic or Latino, or Native Hawaiian or Other Pacific Islander. Non-TMDS are students who reported their race/ethnicity as White. Race and ethnicity were aggregated in this way to protect student privacy and confidentiality."

note_firstgen <- ""
  
  
  #"Note. First Gen = First Generation. Continuing Gen = Continuing Generation. Students whose parents have not completed a four-year college/university degree are considered First Generation. Students with at least one parent who completed a four-year college/university degree are considered Continuing Generation."

#note_residency <- NOT NECESSARY

note_all <- ""
  
  #"Note. TMDS = Traditionally Marginalized Domestic Students. TMDS/Non-TMDS calculated based on race/ethnicity data collected during admissions. TMDS includes students who reported their race/ethnicity as American Indian or Alaska Native, Asian, Black or African American, Hispanic or Latino, or Native Hawaiian or Other Pacific Islander. Non-TMDS are students who reported their race/ethnicity as White. Race and ethnicity were aggregated in this way to protect student privacy and confidentiality. Binary gender is reported due to how this information is collected during admissions and to protect student privacy and confidentiality. First Gen = First Generation. Continuing Gen = Continuing Generation. Students whose parents have not completed a four-year college/university degree are considered First Generation. Students with at least one parent who completed a four-year college/university degree are considered Continuing Generation."
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(IRsex19,
             c_gender,
             "Discussions with Diverse Others: Binary Gender",
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(firstgen,
            c_firstgen,
            "Discussions with Diverse Others: Parent Education Level",
            "Percent of students who \"Often\" or \"Very Often\" had conversations with those differing in...",
            note_firstgen)
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(IRsex19,
                 c_gender,
                 "Quality of Interactions: Binary Gender",
                 "Percent of students who rated their interactions with the following groups as as \"Nearly Excellent\" or \"Excellent\"",
                 note_gender)
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(tmds,
                  ...
                  "Percent of students who rated their interactions with the following groups as \"Nearly Excellent\" or \"Excellent\"",
                  note_tmds)
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(firstgen,
                  c_firstgen,
                  "Quality of Interactions: Parent Education Level",
                  "Percent of students who rated their interactions with the following groups as as \"Nearly\" Excellent or \"Excellent\"",
                  note_firstgen)
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(IRsex19,
                  c_gender,
                   "Supportive Environment: Binary Gender",
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(firstgen,
                  c_firstgen,
                   "Supportive Environment: Parent Education Level",
                   "Percent of students who felt \"Quite a Bit\" or \"Very Much\" that UO emphasized...",
                   note_firstgen)
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(IRsex19,
                  c_gender,
                  "Reflective & Integrative Learning: Binary Gender",
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(firstgen,
                  c_firstgen,
                  "Reflective & Integrative Learning: Parent Education Level",
                  "Percent of students who \"Often\" or \"Very Often\"...",
                   note_firstgen)
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(IRsex19,
                  c_gender,
                  "High-Impact Practices: Binary Gender",
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(firstgen,
                  c_firstgen,
                  "High-Impact Practices: Parent Education Level",
                  "Percent of students who have or are planning to...",
                   note_firstgen)
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(firstgen,
                  c_firstgen,
                  "Career Plans: Parent Education Level",
                  "Percent of students who felt \"Quite a Bit\" or \"Very Much\"...",
                   note_firstgen)
...
cwp_plans_mean <- nsse_idr_long %>% 
  ...
  group_by(studentid,
           ...
           firstgen,
           residency) %>% 
  summarize(stu_avg = mean(as.numeric(resp_num))) %>% # composite not computed if student didn't respond to all items
  ungroup()

cwp_plans_mean %>% 
  dumbbellprep_comp() %>% 
  dumbbellplot_comp("Career Plans: Average Score", 
                    "",
                    "")


#                      "Note. Average score calculated from 5 questions measuring career plans. TMDS = Traditionally Marginalized Domestic Students. TMDS/Non-TMDS calculated based on race/ethnicity data collected during admissions. TMDS includes students who reported their race/ethnicity as American Indian or Alaska Native, Asian, Black or African American, Hispanic or Latino, or Native Hawaiian or Other Pacific Islander. Non-TMDS are students who reported their race/ethnicity as White. Race and ethnicity were aggregated in this way to protect student privacy and confidentiality. Binary gender is reported due to how this information is collected during admissions and to protect student privacy and confidentiality. First Gen = First Generation. Continuing Gen = Continuing Generation. Students whose parents have not completed a four-year college/university degree are considered First Generation. Students with at least one parent who  completed a four-year college/university degree are considered Continuing Generation.")
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(IRsex19,
                  c_gender,
                  "Career Plan Influences: Binary Gender",
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(firstgen,
                  c_firstgen,
                  "Career Plan Influences: Parent Education Level",
                  "Percent of students who felt \"Quite a Bit\" or \"Very Much\" their career plans were influenced by...",
                 "")
...
cwp_inf_mean %>% 
  dumbbellprep_comp() %>% 
  dumbbellplot_comp("Career Influences: Average Score", 
                    "",
                    "")

#"Note. Average score calculated from 5 questions measuring career plan influences. TMDS = Traditionally Marginalized Domestic Students. TMDS/Non-TMDS calculated based on race/ethnicity data collected during admissions. TMDS includes students who reported their race/ethnicity as American Indian or Alaska Native, Asian, Black or African American, Hispanic or Latino, or Native Hawaiian or Other Pacific Islander. Non-TMDS are students who reported their race/ethnicity as White. Race and ethnicity were aggregated in this way to protect student privacy and confidentiality. Binary gender is reported due to how this information is collected during admissions and to protect student privacy and confidentiality. First Gen = First Generation. Continuing Gen = Continuing Generation. Students whose parents have not completed a four-year college/university degree are considered First Generation. Students with at least one parent who  completed a four-year college/university degree are considered Continuing Generation.")
...
# CWP module: frequency of career preparation activities *during courses*
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(IRsex19,
                  c_gender,
                  "Civic Engagement Encouragement: Binary Gender",
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(firstgen,
                  c_firstgen,
                  "Civic Engagement Encouragement: Parent Education Level",
                  "Percent of students who felt \"Quite a Bit\" or \"Very Much\" enouraged to address important civic issues in...",
                 "")
...
civ_encouragement_mean %>% 
  dumbbellprep_comp() %>% 
  dumbbellplot_comp("Civic Engagement Encouragement: Average Score", 
                    "",
                    "")

                      #"Note. Average score calculated from 2 questions measuring civic engagement encouragement. TMDS = Traditionally Marginalized Domestic Students. TMDS/Non-TMDS calculated based on race/ethnicity data collected during admissions. TMDS includes students who reported their race/ethnicity as American Indian or Alaska Native, Asian, Black or African American, Hispanic or Latino, or Native Hawaiian or Other Pacific Islander. Non-TMDS are students who reported their race/ethnicity as White. Race and ethnicity were aggregated in this way to protect student privacy and confidentiality. Binary gender is reported due to how this information is collected during admissions and to protect student privacy and confidentiality. First Gen = First Generation. Continuing Gen = Continuing Generation. Students whose parents have not completed a four-year college/university degree are considered First Generation. Students with at least one parent who  completed a four-year college/university degree are considered Continuing Generation.")
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(IRsex19,
                  c_gender,
                  "Involvement with Campus and Local Issues: Binary Gender",
                  "Percent of students who \"Often\" or \"Very Often\"...",
                  note_gender)
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(firstgen,
                  c_firstgen,
                  "Involvement with Campus and Local Issues: Parent Education Level",
                  "Percent of students who \"Often\" or \"Very Often\"...",
                 "")
...
civ_loc_mean %>% 
  dumbbellprep_comp() %>% 
  dumbbellplot_comp("Involvement with Campus and Local Issues: Average Score", 
                    "",
                    "")


#                      "Note. Average score calculated from 3 questions measuring involvement with campus and local issues. TMDS = Traditionally Marginalized Domestic Students. TMDS/Non-TMDS calculated based on race/ethnicity data collected during admissions. TMDS includes students who reported their race/ethnicity as American Indian or Alaska Native, Asian, Black or African American, Hispanic or Latino, or Native Hawaiian or Other Pacific Islander. Non-TMDS are students who reported their race/ethnicity as White. Race and ethnicity were aggregated in this way to protect student privacy and confidentiality. Binary gender is reported due to how this information is collected during admissions and to protect student privacy and confidentiality. First Gen = First Generation. Continuing Gen = Continuing Generation. Students whose parents have not completed a four-year college/university degree are considered First Generation. Students with at least one parent who  completed a four-year college/university degree are considered Continuing Generation.")
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(IRsex19,
                 c_gender,
                 "Ability with Conflict Resolution: Binary Gender",
...
nsse_idr_long %>% 
  ...
  dumbbells_mult(firstgen,
                 c_firstgen,
                 "Ability with Conflict Resolution: Parent Education Level",
                 "Percent of students who rated their ability with the following as a 4 or 5 (on a scale from 1 = \"Poor\" to 5 = \"Excellent\")",
                 "")
...
```{r gt-dbell-summary-table-cwp-plans-mean}
...
cwp_plans_mean %>% 
  ...
  gt() %>%
  tab_header(title = "Career Plans: Average Score") %>%
  fmt_number(4:7,
             decimals = 2) %>% 
  tab_spanner(
    label = "First Year",
...
```{r gt-dbell-summary-table-cwp-influences-mean}
...
cwp_inf_mean %>% 
  ...
  pivot_wider(names_from = IRclass,
              values_from = c(n, mean, sd)) %>% 
  gt() %>%
  tab_header(title = "Career Plan Influences: Average Score") %>%
  fmt_number(4:7,
             decimals = 2) %>% 
  tab_spanner(
    label = "First Year",
...
```{r gt-dbell-summary-table-civ-encouragement-mean}
...
civ_encouragement_mean %>% 
  ...
  pivot_wider(names_from = IRclass,
              values_from = c(n, mean, sd)) %>% 
  gt() %>%
  tab_header(title = "Civic Engagement Encouragement: Average Score") %>%
  fmt_number(4:7,
             decimals = 2) %>% 
  tab_spanner(
    label = "First Year",
...
```{r gt-dbell-summary-table-civ-localissues-mean}
...
civ_loc_mean %>% 
  ...
  pivot_wider(names_from = IRclass,
              values_from = c(n, mean, sd)) %>% 
  gt() %>%
  tab_header(title = "Involvement with Campus and Local Issues: Average Score") %>%
  fmt_number(4:7,
             decimals = 2) %>% 
  tab_spanner(
    label = "First Year",
...
plot(likdata, type = "bar") + 
  labs(title = "Discussions with Diverse Others: First Year Students by Binary Gender",
...
plot(likdata2, type = "bar") + 
  labs(title = "Discussions with Diverse Others: Senior Students by Binary Gender",
       subtitle = "During the current school year, how often have you had discussions with those differing in...?") + 
  theme(legend.title = element_blank(),
